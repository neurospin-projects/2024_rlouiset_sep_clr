...
from data_augmentation import CelebADataAugmentation
from dataset import celeba_accesories_dataset
...
def train(encoder, train_loader):
    encoder.train()
    train_loss = 0
    for batch_idx, (view, sex_labels, labels) in enumerate(train_loader):
        with torch.no_grad():
...
train_dataset, test_dataset = celeba_accesories_dataset()
train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size)

# instantiate Data Augmentation pipeline
data_aug_pipeline = CelebADataAugmentation()
...
torch.save(encoder, "balanced_celeba_k-jem_sep_clr.pth")
